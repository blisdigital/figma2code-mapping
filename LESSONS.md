...
[LESSON — 2026-05-11] [confirmation]
Situation: Reviewed five external sources targeting the same Figma-to-code problem (UX Collective DESIGN.md+metadata.json, DirectedEdges/specs static schema with paid Pro tier, Aleksandar Masic 5-step MCP+Storybook workflow, Ian Guisard ComponentMD with specialist agents, the broader LinkedIn discussion). All five validate the diagnosis ("Figma file is full of information, not specifications") and none have v3.3's combination of drift loop + token verdicts + atomic-design + element-frame + sister-skill.
What worked: Sharper opening line from Guisard ("information, not specifications") communicates the gap better than our previous Vision opener. Adopted in both SKILL.md § Vision and README § Vision. Plus surfaced a real developer-pain motivation (MCP token cost complaints, echoed by Nathan Curtis "why spend all those tokens on MCP") — addressed by documenting frozen-specs workflow (commit figma-context/ instead of gitignore) without adding new mechanics.
Proposal: When iterating, read the field but resist adopting their mechanics unless they solve a problem v3.x doesn't already. Validation > imitation. Pitch references kept in a private scratchpad (figma2code-pitch-references.md), not in skill repo — same scope discipline as v3.2 closed PR #6 enforced.
