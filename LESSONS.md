...
Situation: `drifts.md` was a generic filename that did not signal which skill owns the artifact. With sister skill `figma-to-code-implement` reading and (per controlled-write exception) updating drift status, the filename ambiguity grew: who is the source of truth?
What worked: Renamed `templates/drifts.md` → `templates/drifts-mapping.md`. Signal in the filename: mapping-side is the canonical writer and owner; implement-skill reads + writes status under the documented exception. Single file, clearer name. Historical LESSONS entries keep the original `drifts.md` term — append-only history is preserved.
Proposal: Future cross-skill artifacts get explicit skill-suffix naming if both skills touch them. Migration for existing projects: rename `docs/drifts.md` to `docs/drifts-mapping.md` after `git pull` of the skill repo.
